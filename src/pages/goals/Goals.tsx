...
const Goals = () => {
  ...
  const categories = ['Personal', 'Fitness', 'Learning', 'Career', 'Financial', 'Health', 'Development'];
  ...
  const handleGoalUpdate = (id: string, progress: number) => {
          ...
          <TabsContent value="all" className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {goals.map(goal => (
              <GoalCard
                ...
                onUpdateProgress={(progress) => handleGoalUpdate(goal.id, progress)}
              />
            ))}
          </TabsContent>
